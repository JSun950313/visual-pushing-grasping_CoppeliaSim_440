...
class Robot(object):
    ...
    def restart_sim(self):
        ...
        self.UR5_target_handle = self.sim_sim.getObject('/UR5_target')
        self.sim_sim.stopSimulation()

        self.sim_sim.setObjectPosition(self.UR5_target_handle, -1, (-0.5, 0, 0.3))
        self.sim_sim.startSimulation()

        time.sleep(1)
        self.RG2_tip_handle = self.sim_sim.getObject('/UR5_tip')
    ...
    def get_obj_positions_and_orientations(self):
        ...
        for object_handle in self.object_handles:
            object_position = self.sim_sim.getObjectPosition(object_handle, -1)
            object_orientation = self.sim_sim.getObjectOrientation(object_handle, -1)
            obj_positions.append(object_position)
    ...
    def reposition_objects(self, workspace_limits):
        ...
        for object_handle in self.object_handles:
            ...
            object_orientation = [2 * np.pi * np.random.random_sample(), 2 * np.pi * np.random.random_sample(),
                                  2 * np.pi * np.random.random_sample()]
            self.sim_sim.setObjectPosition(object_handle, -1, object_position)
            self.sim_sim.setObjectOrientation(object_handle, -1, object_orientation)
            time.sleep(2)
